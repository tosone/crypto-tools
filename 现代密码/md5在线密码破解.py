...
def wmd5(md5):
    ...
    try:
        ...
        if a['status'] == 'success':
            print u'[-] site: http://www.wmd5.com/\t\t\t查询的结果是：' + str(a['md5text'])
        else:
            print u'[-] site: http://www.wmd5.com/\t\t\t没有查到'
    except urllib2.URLError:
        print '[+] site: %s \t\t\t[+] Error: seems to be down' % url
...
def myaddr(passwd):
    ...
    try:
        ...
        if match:
            print u'[-] site: %s\t\t查找的结果是：%s' % (site, match.group(2))
        else:
            print u'[-] site: %s\t\tPassword: Not found' % site
    except urllib2.URLError:
